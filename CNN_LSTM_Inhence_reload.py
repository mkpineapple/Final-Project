...
# loading news
News_Data_train = load_data_new.News_Data(x_news_train, dictionary_word, word_limit, day_news_limit, midterm_news_limit,
                                          long_news_limit)
News_Data_val = load_data_new.News_Data(x_news_val, dictionary_word, word_limit, day_news_limit, midterm_news_limit,
                                        long_news_limit)
News_Data_test = load_data_new.News_Data(x_news_test, dictionary_word, word_limit, day_news_limit, midterm_news_limit,
                                         long_news_limit)
...
with tf.device('/device:GPU:0'):
    with tf.Session(config=tf.ConfigProto(log_device_placement=True,
        ...
        saver = tf.train.import_meta_graph('save_LSTM_CNN_Inhence_model/cnn_lstm_model_v1.ckpt.meta')
        saver.restore(sess, "save_LSTM_CNN_Inhence_model/cnn_lstm_model_v1.ckpt")
